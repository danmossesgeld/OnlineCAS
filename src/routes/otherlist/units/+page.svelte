...
<div class="bg-white rounded-2xl shadow-xl p-8">
  <h1 class="text-2xl font-bold mb-2 flex items-center gap-2"><iconify-icon icon="material-symbols:straighten-rounded" width="28" height="28" /> Units</h1>
...
{#if showModal}
  <div class="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
    <div class="bg-white rounded-xl shadow-lg p-6 w-full max-w-sm">
      <h2 class="text-xl font-bold mb-4">{editingItem ? 'Edit' : 'Add'} Unit</h2>
      <input class="input input-bordered w-full mb-2" placeholder="Unit Name" bind:value={formData.name} />
      {#if errorMsg}
