<script lang="ts">
  ...
  const subCollectionName = 'costcenters';
  const collectionPath = 'otherlist/costcenters';  // This will be mapped to listdatabase/otherlist/costcenters

  // ListContainer configuration
  const documentType = 'cost center';
  const title = 'Cost Centers';
  const subtitle = 'Manage cost centers used on APV expense lines';
  const primaryColorClass = 'teal';
...
<MasterListContainer
  ...
  >
    <svelte:fragment slot="additionalActions" let:row>
      <button class="btn btn-ghost btn-xs" aria-label="Edit cost center" on:click={() => handleEdit(row)}><iconify-icon icon="material-symbols:edit-outline" width="20" height="20"></iconify-icon></button>
      <button class="btn btn-ghost btn-xs" aria-label="Delete cost center" on:click={() => handleDelete(row)}><iconify-icon icon="material-symbols:delete-outline" width="20" height="20"></iconify-icon></button>
    </svelte:fragment>
  </MasterListContainer>

{#if showModal}
  <ModalForm
    title={editingItem ? 'Edit Cost Center' : 'Add Cost Center'}
