<script lang="ts">
  ...
  const subCollectionName = 'discounts';
  const collectionPath = 'otherlist/discounts';  // This will be mapped to listdatabase/otherlist/discounts
  
  // ListContainer configuration
  const documentType = 'discount';
  const title = 'Discounts';
  const subtitle = 'Manage discount options for sales transactions';
  const primaryColorClass = 'amber';
