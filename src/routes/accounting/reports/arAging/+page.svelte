<script lang="ts">
  ...
  import { getARAgingData, type AgingItem } from '$lib/utils/reportingService';
  ...
  async function generateReport() {
    try {
      loading = true;
      error = null;
      reportData = await getARAgingData(asOfDate);
      loading = false;
    } catch (err) {
  ...
  function handleExport(event: CustomEvent) {
    // Create CSV content
    let csvContent = "Accounts Receivable Aging Report\n";
    ...
    reportData.customers.forEach(customer => {
      csvContent += `"${customer.name}",${customer.totalAmount}`;
      customer.buckets.forEach(bucket => {
        csvContent += `,${bucket.amount}`;
      });
      csvContent += "\n";
      
      // Add detail rows if showing details
      if (showDetails && customer.documents.length > 0) {
    ...
    link.setAttribute('download', event.detail.filename || `ar_aging_as_of.csv`);
...
<ReportContainer
  ...
  description="Track outstanding customer invoices by age"
  {loading}
  {error}
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            {#each reportData.customers as customer}
              <tr>
                ...
                {#each customer.buckets as bucket}
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-right">
              ...
              {#if showDetails && customer.documents.length > 0}
                {#each customer.documents as document}
                  <tr class="bg-gray-50">
