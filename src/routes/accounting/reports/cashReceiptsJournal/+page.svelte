<script lang="ts">
  ...
  import { getCashReceiptsJournalData, type CashJournalRow, type DateRange } from '$lib/utils/reportingService';
  ...
  async function generateReport() {
    try {
      loading = true;
      error = null;
      const result = await getCashReceiptsJournalData(dateRange);
      rows = result.rows;
      total = result.total;
      loading = false;
    } catch (err) {
  ...
  function handleExport(event: CustomEvent) {
    let csvContent = 'Date,Receipt #,Customer,Account,Amount\n';
    rows.forEach((r) => {
    ...
    link.setAttribute('download', event.detail.filename || 'cash_receipts_journal.csv');
...
<ReportContainer
  ...
  description="Posted customer Receipts for the period, with the offsetting account credited"
  {loading}
  {error}
  parameterType="dateRange"
  startDate={dateRange.startDate}
  endDate={dateRange.endDate}
  reportId="cashReceiptsJournal"
  filename="cash_receipts_journal"
  ...
  <div slot="report-content">
    {#if rows.length === 0}
      <div class="text-center py-10">
        <p class="text-sm" style="color: var(--color-neutral-600);">No Receipts found for this period.</p>
      </div>
    {:else}
        ...
        <table class="min-w-full text-sm">
          <thead>
            <tr>
              ...
              <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider" style="color: var(--color-neutral-500); border-bottom: 1px solid var(--color-neutral-200);">Receipt #</th>
