...
Read this section before assuming anything else in the codebase is production-safe. **Items 1, 3–6, 8–11 below were fixed on 2026-08-14** (see the "Fixed" note under each); items 2 and 7 are still open and require a product/scope decision before anyone implements them (see the note after the list).
...
10. **Sales Invoice could post an unbalanced journal entry.** `createSalesInvoiceJournalEntry` credited "Sales Revenue" at `grossAmount / 1.12` — i.e. it assumed the *entire* invoice was vatable — while Output VAT was correctly computed on the vatable portion only. Any invoice with a zero-rated or exempt line (including a line simply left at the default blank tax type, which `getTaxCategory` classifies as exempt) posted debits ≠ credits, off by ~10.71% of the non-vatable amount. **Fixed** — the Sales Invoice form now also computes and saves `vatableGross` (pre-discount gross of vatable lines only); `createSalesInvoiceJournalEntry` uses it to split the "Sales Discount" debit and "Sales Revenue" credit to the vatable portion only, and adds separate "Zero-Rated Sales" / "VAT-Exempt Sales" credit lines (same `sales-revenue`/`Sales` account, distinct `lineDescription`) for the other two categories, which are already net-of-discount and VAT-exclusive. Verified balanced across 11 scenarios (all-vatable, all-exempt, all-zero-rated, mixed, with discount, with withholding, cash sale). The journal entry header's `totalDebit`/`totalCredit` were also changed to sum the actual `lines` rather than always mirroring `totalDue` (harmless before since reports read `lines` directly, but the header numbers shown in Audit Trail were wrong whenever a discount or withholding line existed).
11. **Receiving Report's journal entry never fired.** `formData.status` was hardcoded to `'Draft'` at form init and nothing in the form ever transitioned it to `'Posted'` — the JE-generation branch (gated on `status === 'Posted'`) was permanently dead code reachable only by hand-editing the Firestore document. Same root cause as item 3 above (APV), just not caught on that pass. **Fixed** — `saveReceivingReport` now takes a `status` parameter mirroring the APV/Sales Invoice pattern; the primary button posts (`'Posted'`), a new secondary "Save as Draft" button (replacing the old "Cancel", which just duplicated `FormLayout`'s existing back button) saves as `'Draft'`.
