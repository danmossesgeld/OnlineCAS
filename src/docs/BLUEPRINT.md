...
| `withholdingTax.ts` | `WITHHOLDING_TAX_OPTIONS` — the `[{label:'Select Withholding Tax',value:''},{label:'1%',...},{label:'2%',...}]` list every transaction type with a real withholding concept (Sales Invoice, Credit Memo, APV) passes to `FormFooter`'s `withholdingTaxOptions` prop. Was three independently duplicated array literals before. |
